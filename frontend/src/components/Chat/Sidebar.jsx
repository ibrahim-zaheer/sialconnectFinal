...
const Sidebar = () => {
  ...
  // Sort so users with unread messages appear first
filteredUsers.sort((a, b) => {
  const aUnread = unreadMessages[a._id] || 0;
  const bUnread = unreadMessages[b._id] || 0;

  // If both have same unread, keep order
  if (aUnread === bUnread) return 0;

  // Otherwise, user with more unread comes first
            ...
            <div className="hidden md:block text-left min-w-0 flex-1">
              ...
              <div className="flex justify-between items-center mt-1">
                ...
                {unreadMessages[user._id] > 0 && (
                  <span className="bg-primary-500 text-white rounded-full min-w-5 h-5 px-1 flex items-center justify-center text-xs">
...
//   const newMessage = user.messages?.some(
//     (message) => !message.read && message.receiverId === user._id
//   );
//   return newMessage ? "bg-primary-100" : "hover:bg-neutral-50";
// };

// const sortUsersByNewMessage = (users) => {
//   return users.sort((a, b) => {
//     const aHasNewMessage = a.messages?.some((message) => !message.read && message.receiverId === a._id);
//     const bHasNewMessage = b.messages?.some((message) => !message.read && message.receiverId === b._id);
//     return (bHasNewMessage ? 1 : 0) - (aHasNewMessage ? 1 : 0); // New message goes to the top
//   });
// };
...
//       {/* Contacts list */}
//       <div className="flex-1 overflow-y-auto">
//         {sortUsersByNewMessage(filteredUsers).map((user) => (
//           <button
//             key={user._id}
//             onClick={() => setSelectedUser(user)}
//             className={`w-full px-3 py-3 flex items-center gap-3 transition-colors ${getUserStatus(user)} border-b border-neutral-100`}
//           >
//             <div className="relative">
//               <img
//                 src={user.profilePicture || "/default-avatar.png"}
