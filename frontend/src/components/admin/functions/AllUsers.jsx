...
const AllUsers = () => {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const ageMatch =
      !userAge || !min ? true : userAge >= min && userAge <= max;

    return searchMatch && cityMatch && ageMatch;
  });

  const suspendUser = async (userId) => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.put(`/api/admin/suspend/${userId}`, {}, {
        headers: {
  ...
  const reactivateUser = async (userId) => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.put(`/api/admin/reactivate/${userId}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      alert(res.data.message);
  ...
  const toggleUserStatus = async (userId) => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.put(`/api/admin/toggle-status/${userId}`, {}, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
        ...
        <tbody>
          {filteredUsers.map((user) => (
            <tr key={user._id} className="border-b">
              ...
              <td className="py-2 px-4">
  <span
    className={`badge ${user.status === "suspended" ? "badge-error" : "badge-success"}`}
  >
    {user.status}
  </span>
</td>
